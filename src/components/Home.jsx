...
const Home = (props) => {
  ...
  useEffect(() => {
    setAll({
      edu: props.allData.edu,
      skill: props.allData.skill,
      project: props.allData.project,
      noti: props.allData.noti,
    });
  }, []);
  return (
    <motion.div
      ...
      className="w-full sm:h-[calc(100% - 40px)] flex item-center justify-center sm:flex-row flex-col-reverse"
    >
      {tag == null ? (
          ...
          if (list.id !== "id") {
            return (
              <>
                <div
                  key={list.id}
                  className="left  sm:h-full sm:w-1/2 h-[calc(30%-40px)]  w-full flex items-center justify-center relative"
                >
                  <img
                    src={
                      "https://17akshar.github.io/portfolio-image-hosting/" +
                      list.main_img_id
                    }
                    alt=""
                    className="sm:h-[70%] h-[90%] relative"
                  />
                </div>
                <div className="right sm:h-full sm:w-1/2 h-[60%] w-full flex items-center justify-center flex-col">
                  <h1 className="sm:text-6xl text-4xl font-semibold">
                    <span className="text-[#FA5A00]">A</span>kshar{" "}
                    <span className="text-[#FA5A00]">P</span>armar
                  </h1>
                  <p className="sm:text-3xl text-xl text-[#FA5A00] font-semibold sm:mt-3 mt-2 tracking-wide">
                    {list.name_desc}
                  </p>
                  <p className="text-[#FFFFFF] sm:text-3xl text-center sm:mt-4 sm:px-40 px-10 mt-5">
                    {list.description}
                  </p>
                  <a
                  ...
                  >
                    <button className="bg-[#FA5A00] sm:px-8 sm:py-2 px-6 py-2 sm:text-xl text-lg rounded-lg mt-10">
                      Resume
                    </button>
                  </a>
